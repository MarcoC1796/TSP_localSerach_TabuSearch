...
import random
def tsp_local_search(graph, max_iter=float('inf'), max_visits_in_neigh = float('inf'), probability_to_visit = 1):
    ...
    while True:
        ...
        for start in range(0, node_count - 1): 
            for end in range(start + 2, node_count):
                ...
                if j > max_visits_in_neigh or n < (1-probability_to_visit)  or (end - start) > node_count - 2:
                    continue
...
def tsp_tabu_search(graph, max_failures=10, max_tabu_elements=0, 
                    max_iter=float('inf'), max_visits_in_neigh = float('inf'), probability_to_visit = 1):
    graph.set_current_path(graph.initial_path)
...
def tsp_local_search_random(graph, visits, max_iter=float('inf'), max_visits_in_neigh = float('inf'), probability_to_visit = 1):
    graph.set_current_path(graph.initial_path)
    node_count = graph.node_count
    reversals = []
    i=0
    while True:
        ...
        for _ in range(visits):
            start = random.randint(0, node_count - 2)
            end = random.randint(start + 2, node_count)
            n = random.random()
...
def tsp_tabu_search_random(graph, visits, max_failures=10, max_tabu_elements=0, 
                    max_iter=float('inf'), max_visits_in_neigh = float('inf'), probability_to_visit = 1):
    graph.set_current_path(graph.initial_path)
    best_global_path = graph.current_path
    best_global_path_length = graph.path_length(best_global_path)
